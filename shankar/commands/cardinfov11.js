...
module.exports.config = {
  ...
  description: "Facebook ke liye user ka info card v11",
...
module.exports.run = async ({ api, event, Users, Threads, args }) => {
  try {
    ...
    ctx.textAlign = "start";
    var fontSize = 20;
    ctx.fillText(`» Naam: ${name}`, 455, 172);
    ctx.fillText(`» Ling: ${gender}`, 455, 208);
    ctx.fillText(`» Followers: ${follower}`, 455, 244);
    ctx.fillText(`» Rishta: ${relationship_status}`, 455, 281);
    ctx.fillText(`» Janmdin: ${birthday}`, 455, 320);
    ctx.fillText(`» Jagah: ${location}`, 455, 357);
