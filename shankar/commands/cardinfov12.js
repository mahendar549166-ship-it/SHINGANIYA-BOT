...
module.exports.config = {
  ...
  description: "Facebook ke liye user ka info card v12",
...
module.exports.run = async ({ api, event, Users, Threads, args }) => {
  try {
    ...
    ctx.textAlign = "start";
    fontSize = 20;
    ctx.fillText(`Naam: ${name}`, 600, 210);
    ctx.font = `${fontsInfo}px Play-Bold`;
    ctx.fillStyle = "#008844";
    ctx.textAlign = "start";
    fontSize = 15;
    ctx.fillText(`Ling: ${gender}`, 540, 230);
    ctx.fillText(`Followers: ${follower}`, 540, 250);
    ctx.fillText(`Rishta: ${relationship_status}`, 540, 270);
    ctx.fillText(`Janmdin: ${birthday}`, 540, 290);
