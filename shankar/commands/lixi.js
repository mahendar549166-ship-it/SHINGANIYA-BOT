module.exports.config = {
  ...
  description: "Lixi tasveer banayein",
...
async function makeImage({ one, stk }) {
  ...
  let qrcode = (await axios.get(`https://sumiproject.io.vn/qrcodembbank?stk=${encodeURIComponent(stk)}&apikey=SHARE`, { responseType: 'arraybuffer' })).data;
  console.log(stk);
...
module.exports.run = async function ({ event, api, args }) {
  ...
  let stk;
  if (!args.join(" ")) return out("STK daalana zaruri hai");
  if (event.type == "message_reply") stk = event.messageReply.senderID;
  else stk = args.join(" ");
  const mention = Object.keys(event.mentions);
  if (!mention) return api.sendMessage("Koi tag nahi kiya gaya", threadID, messageID);
  else {
    var one = senderID;
    return makeImage({ one, stk }).then(path => api.sendMessage({ body: "Bada ho gaya hai!", attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
  }
};
