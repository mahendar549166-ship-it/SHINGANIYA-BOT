module.exports.config = {
  ...
  description: "Kisi ko chupke se chumne ka khel",
...
module.exports.run = async function ({ event, api, args, Currencies }) {
  ...
  if (!two) return api.sendMessage("Kripaya 1 vyakti ko tag karein", threadID, messageID);
  else {
    return makeImage({ one, two }).then(path => api.sendMessage({
      body: `${ae[Math.floor(Math.random() * ae.length)]}\n Chupke se chumne ke baad aapke prati sneh ${hc}% hai\n + ${((hc) * rd)} $`,
