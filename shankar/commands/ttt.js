module.exports.config = {
    ...
    description: "AI ke saath caro khelein",
...
function move(x, y, data) {
    ...
    } else return "⚠️ Yeh khana pehle se chinhit hai!";
...
module.exports.handleReply = async function({ event, api, handleReply }) {
    ...
    if(!isNaN(number) && number > 0 && number < 10) {
        ...
        if(checkGameOver(data)) {
            var gayban = ["murgi 😎", "naya kheladi 😎", "kuch nahi aata 😎", "thodi si practice chahiye 😎", "bohot weak 😎", "aasaan khel tha 😎","buddhu jaisa 😏"];
            if(checkAIWon(data)) lmao = `[ TIC TAC TOE - GLOW ]\n────────────────────\n🏆 Parinaam: Aap haare\n✏️ Aap ${gayban[Math.floor(Math.random() * gayban.length)]}\n⏰ Samay: ${Tm}`;
            else if(checkPlayerWon(data)) lmao = `[ TIC TAC TOE - GLOW ]\n────────────────────\n🏆 Parinaam: Aap jeete\n⏰ Samay: ${Tm}`;
            else lmao = `[ TIC TAC TOE - GLOW ]\n────────────────────\n🏆 Parinaam: Barabari\n⏰ Samay: ${Tm}`;
            global.moduleData.tictactoe.delete(threadID);
        }
        var msg = lmao !== "" ? lmao : temp == undefined ? `[ TIC TAC TOE - GLOW ]\n────────────────────\n⚠️ Khana chinhit karne ke liye number reply karein\n────────────────────` : temp;
    ...
    } else return api.sendMessage("⚠️ Khana ka number galat hai!", threadID, messageID);
}
...
module.exports.run = async function ({ event, api, args }) {
    ...
    let newData;
    if (args.length == 0) return api.sendMessage(`⚠️ X ya O chunein\nUdaharan: ${prefix}ttt X/O`, threadID, messageID);
    if (args[0].toLowerCase() == "delete") {
        global.moduleData.tictactoe.delete(threadID);
        return api.sendMessage("⚠️ Khel ka board mita diya gaya!", threadID, messageID);
    }
    if (args[0].toLowerCase() == "continue") {
        if (!data.gameOn) return api.sendMessage("⚠️ Koi data nahi hai! Naya khel shuru karne ke liye " + concak + " x/o ka istemal karein", threadID, messageID);
        return api.sendMessage({ body: "⚠️ Khana chinhit karne ke liye number reply karein", attachment: await displayBoard(data)}, threadID, (error, info) => {
            global.client.handleReply.push({
                name: this.config.name,
                author: senderID,
                messageID: info.messageID
    ...
    if (!data.gameOn) {
        ...
        if (abc !== "x" && abc !== "o") return api.sendMessage("⚠️ X ya O chunein", threadID, messageID);
        if (abc == "o") {
            newData = startBoard({ isX: false, data, threadID });
            api.sendMessage({ body: "⚠️ Aap pehle chalein!\n✏️ Khana chinhit karne ke liye number reply karein", attachment: await displayBoard(newData)}, threadID, (error, info) => {
                global.client.handleReply.push({
                    name: this.config.name,
                    author: senderID,
                    messageID: info.messageID
        ...
        if (abc == "x") {
            ...
            AIStart(newData);
            api.sendMessage({ body: "⚠️ AI pehle chalega!\n✏️ Khana chinhit karne ke liye number reply karein", attachment: await displayBoard(data)}, threadID,(error, info) => {
                global.client.handleReply.push({
                    name: this.config.name,
                    author: senderID,
                    messageID: info.messageID
    ...
    } else return api.sendMessage("⚠️ Is group mein pehle se ek khel chal raha hai\n✏️ Istemal karein:\n" + concak + " continue -> jari rakhein\n" + concak + " delete -> mita dein", threadID, messageID);
}
