...
async function getData(url) {
  try {
    ...
    if (!k_exp || !k_token) throw new Error('Token prapt karne mein asafal.');
...
this.config = {
    ...
    description: 'Facebook link ka pata lagte hi automatic download karein',
    ...
    for (const str of urls) {
        if (/facebook\.com\//.test(str)) {
          ...
          if (res && res.url) {
            ...
            // send({body: 'Video download nahi ho saka.'});
          }
        }
    }
};
