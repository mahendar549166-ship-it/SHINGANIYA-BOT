module.exports.config = {
	...
	hasPermssion: 0,
	description: "Prashno ke jawab dein",
...
module.exports.handleReaction = ({ api, event, handleReaction }) => {
	...
	if (response == handleReaction.answer) api.sendMessage("⚡️Aapne sahi jawab diya!!!", event.threadID, () => {
		setTimeout(function(){ api.unsendMessage(handleReaction.messageID); }, 5000);
	});
	else api.sendMessage("⚡️Aapne galat jawab diya!!!", event.threadID);
...
module.exports.run = async ({ api, event, args }) => {
	...
	if (!fetch.data) return api.sendMessage("⚡️Server ke vyast hone ke kaaran prashn nahi mil saka", event.threadID, event.messageID);
	...
	return request(encodeURI(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=vi&dt=t&q=${decode}`), (err, response, body) => {
		if (err) return api.sendMessage("⚡️Koi error ho gaya!", event.threadID, event.messageID);
		var retrieve = JSON.parse(body);
		var text = '';
		retrieve[0].forEach(item => (item[0]) ? text += item[0] : '');
		var fromLang = (retrieve[2] === retrieve[8][0][0]) ? retrieve[2] : retrieve[8][0][0]
		return api.sendMessage(`⚡️Yeh prashn aapke liye hai:\n- ${text}\n\n   👍: True       😢: False`, event.threadID, async (err, info) => {
			...
			if (data.answerYet !== 1) {
				api.sendMessage(`⚡️Time out!! Sahi jawab hai ${fetch.data.results[0].correct_answer}`, event.threadID, info.messageID);
