module.exports.config = {
  ...
  hasPermssion: 0,
  credit: "Trúc cute :)",
  description: "Bye bolne par sticker bhejo",
  commandCategory: "System",
  usages: "[text]",
  cooldowns: 0
}

// Event handler for bye keywords
module.exports.handleEvent = async ({ event, api, Users }) => {
  let KEY = ["bye", "bai", "byee", "off", "pai", "paii"];
  ...
  if (typeof thread["bye"] == "undefined" || thread["bye"] == false) return;
    ...
    if (event.body && KEY.includes(event.body.toLowerCase()) !== false) {
      ...
      let session = (
        hours > 0001 && hours <= 400 ? "subah ki shuruaat" :
        hours > 401 && hours <= 700 ? "subah jaldi" :
        hours > 701 && hours <= 1000 ? "subah" :
        hours > 1001 && hours <= 1200 ? "dopahar" :
        hours > 1201 && hours <= 1700 ? "shaam" :
        hours > 1701 && hours <= 1800 ? "shaam ka waqt" :
        hours > 1801 && hours <= 2100 ? "raatri" :
        hours > 2101 && hours <= 2400 ? "der raat" :
        "error"
      );
      ...
      let msg = {
        body: `[🥨] → Alvida ${name}\n[💓] → Tumhe ek ${session} ${text} din ki shubhkamna\n[🌚] → Jaldi wapas aao bot ke saath interact karne!\n[🌹] → I love ${name}\n[🕰] → Abhi ka samay: ${moment().tz("Asia/Kolkata").format("HH:mm:ss || DD/MM/YYYY")}`,
        mentions
      };
