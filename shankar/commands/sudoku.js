let z = z => `9 numbers ke liye input karein row ${z} ke, agar nahi hai toh 0 daalein`,
...
async function handleReply({ api: { sendMessage, unsendMessage }, event: { args, senderID, threadID, messageID, messageReply }, handleReply: { x } }) {
    let send = (a, b) => sendMessage(a, threadID, b, messageID)
try {
    if(args.length != 9 || !args.every(a => /[0-9]/.test(a))) return send('9 elements 0 se 9 tak ke hone chahiye', (a, b) => setTimeout(() => unsendMessage(b.messageID), 2000))
...
module.exports = {
    ...
    config: {
        ...
        description: 'Sudoku ka samadhan karein',
