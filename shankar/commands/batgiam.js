/**
 * @author ProCoderMew
 * @warn Code ya credits edit mat karein
 */
...
module.exports.config = {
    ...
    description: "Do logon ke avatar ke sath mazaakiya image banayein",
...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage("Kripya 1 vyakti ko tag karein", threadID, messageID);
    else {
        var one = senderID, two = mention;
        return makeImage({ one, two }).then(path => api.sendMessage({ 
            body: "Badhai ho, tum sarkari naukri mein shamil ho gaye " + tag + '\nMaza karo 😆',
            mentions: [{
                tag: tag,
                id: mention
            }],
