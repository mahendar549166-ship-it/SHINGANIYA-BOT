module.exports.config = {
    ...
    usages: "[unban/ban/search] [ID ya text]",
    cooldowns: 5
};
...
module.exports.languages = {
    "vi": {
        ...
        "banSuccess": "[📌 Ban User 💸] User ko safalta se ban kar diya gaya: %1",
        "unbanSuccess": "[🎀 Unban User 💎] User ka ban safalta se hata diya gaya: %1",
        "banCommandSuccess": "[🔰 BanCommand User 🔰] User ke liye alag command ban safalta se kar diya gaya: %1",
        "unbanCommandSuccess": "[🍑 UnbanCommand User 🎲] User ke liye %1 command ka ban safalta se hata diya gaya: %2",
        "errorReponse": "%1 Aapki request puri nahi ki ja sakti",
        "IDNotFound": "%1 Aapne jo ID dala hai woh database mein nahi hai",
        "existBan": "[📌 Ban User 💸] User %1 pehle se hi ban hai %2 %3 se",
        "notExistBan": "[🎀 Unban User 💎] Aapne jo user dala hai woh pehle se ban nahi hai",
        "missingCommandInput": "%1 Ban karne wala command khali nahi hona chahiye!",
        "notExistBanCommand": "[🍑 UnbanCommand User 🎲] Aapne jo user ID dala hai us par koi command ban nahi hai",

        "returnBan": "[📌 Ban User 💸] Aapne is user ko ban karne ka anurodh kiya hai 📵:\n- Ban karne wale user ka ID aur naam 💈: %1%2\n\n❮ Is message par reaction karein taki kaaryawahi puri ho ❯",
        "returnUnban": "[🎀 Unban User 💎] Aapne is user ka ban hatane ka anurodh kiya hai ❗:\n- Ban hatane wale user ka ID aur naam 💌: %1\n\n❮ Is message par reaction karein taki kaaryawahi puri ho ❯",
        "returnBanCommand": "[🔰 BanCommand User 🔰] Aapne is user ke liye alag command ban karne ka anurodh kiya hai 🔒:\n- Ban karne wale user ka ID aur naam 📌: %1\n- Ban karne wale commands 🛠: %2\n\n❮ Is message par reaction karein taki kaaryawahi puri ho ❯",
        "returnUnbanCommand": "[🍑 UnbanCommand User 🎲] Aapne is user ke liye alag command ka ban hatane ka anurodh kiya hai 🎀:\n- Ban hatane wale user ka ID aur naam 🔗: %1\n- Ban hatane wale commands 📜: %2\n\n❮ Is message par reaction karein taki kaaryawahi puri ho ❯",

        "returnResult": "Yeh hai aapka parinaam: \n",
        "returnNull": "Aapke input ke aadhar par koi parinaam nahi mila!",
        "returnList": "[ User List ]\nIs samay %1 users ban hain, yahan %2 users hain\n\n%3",
        "returnInfo": "[ Info User ] Yahan aapke dhoondhe hue user ki kuch jankari hai:\n- User ka ID aur naam: %1n- Ban hai?: %2 %3 %4\n- Command ban hai?: %5"
    },
    "en": {
        "reason": "Reason",
        "at": "At",
...
module.exports.run = async ({ event, api, args, Users, getText }) => {
    ...
    switch (type) {
        ...
        case "-i": {
            ...
            const nameTarget = global.data.userName.get(targetID) || await Users.getNameUser(targetID);
            return api.sendMessage(getText("returnInfo", `${targetID} - ${nameTarget}`, ((!dateAdded) ? "HAAN" : "NAHI"), ((reason) ? `${getText("reason")}: "${reason}"` : ""), ((dateAdded) ? `${getText("at")}: ${dateAdded}` : ""), ((commandBanned) ? `HAAN: ${(commandNeedBan.length == global.client.commands.size) ? getText("allCommand") : commandNeedBan.join(", ")}` : "NAHI")), threadID, messageID);
        }
    }
}
