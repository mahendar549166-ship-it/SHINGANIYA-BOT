...
module.exports.config = {
	...
	description: "Bot ke account par nayi post banayein",
...
module.exports.run = async ({ event, api }) => {
	...
	return api.sendMessage(`Is post ko dekhne wale ka chayan karein\n1. Sabhi\n2. Dost\n3. Sirf main`, threadID, (e, info) => {
...
module.exports.handleReply = async ({ event, api, handleReply }) => {
	...
	if (type === "whoSee") {
		if (!["1", "2", "3"].includes(body)) return api.sendMessage('Kripya upar ke 3 vikalpon mein se 1 chunein', threadID, messageID);
		formData.input.audience.privacy.base_state = body == 1 ? "EVERYONE" : body == 2 ? "FRIENDS" : "SELF";
		api.unsendMessage(handleReply.messageID, () => {
			api.sendMessage(`Is message ka jawab post ke content ke sath dein, agar khaali rakhna hai toh 0 reply karein`, threadID, (e, info) => {
				global.client.handleReply.push({
					name: this.config.name,
					messageID: info.messageID,
					author: senderID,
		...
		api.unsendMessage(handleReply.messageID, () => {
			api.sendMessage(`Is message ka jawab photo ke sath dein (ek se zyada photo bhej sakte hain, agar photo nahi post karna toh 0 reply karein)`, threadID, (e, info) => {
				global.client.handleReply.push({
					name: this.config.name,
					messageID: info.messageID,
					author: senderID,
		...
		api.unsendMessage(handleReply.messageID, () => {
			api.sendMessage(`Post banana shuru kar raha hai....`, threadID, (e, info) => {
				global.client.handleReply.push({
					name: this.config.name,
					messageID: info.messageID,
					author: senderID,
		...
		api.httpPost('https://www.facebook.com/api/graphql/', form, (e, info) => {
			...
			try {
				...
				return api.sendMessage(`» Post safalta se banayi gayi\n» postID: ${postID}\n» urlPost: ${urlPost}`, threadID, messageID);
			} catch (e) {
				console.error(e);
				return api.sendMessage(`Post banana fail hua, kripya baad mein koshish karein`, threadID, messageID);
			}
		});
	}
};
