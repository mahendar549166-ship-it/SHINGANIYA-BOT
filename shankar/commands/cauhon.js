module.exports.config = {
    ...
    description: "Apne chahne wale dost ko shadi ka prastaav dein",
...
module.exports.run = async function ({ event, api, args, client }) {
    ...
    if (!mention) return api.sendMessage("[⚜️]→ Kripya 1 vyakti ko tag karen", threadID, messageID);
    else {
        var one = senderID, two = mention;
        return makeImage({ one, two }).then(path => api.sendMessage({ 
            body: "💖 Main " + tag + " se pyar karta hoon 💖\nKya tum mujhse shadi karoge?",
            mentions: [{
                tag: tag,
                id: mention
            }],
