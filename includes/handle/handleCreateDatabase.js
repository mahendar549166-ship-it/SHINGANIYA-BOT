  ...
  return async function ({ event }) {
    ...
    try {
      if (!allThreadID.includes(stringThreadID) && isGroup) {
        ...
        for (const singleData of threadInfo.userInfo) {
          ...
          try {
            if (allUserID.includes(stringUserID)) {
              ...
              logger(`New User Loaded: ${stringUserID} | ${singleData.name}`, "[ DATABASE ]");
            }
          } catch (e) {
            console.log(e);
          }
        ...
        logger(`New Group Loaded: ${stringThreadID} | ${setting.threadName}`, "[ DATABASE ]");
      }
      ...
      if (!allUserID.includes(stringSenderID) || !userName.has(stringSenderID)) {
        ...
        logger(`New User Loaded: ${stringSenderID} | ${infoUsers.name}`, "[ DATABASE ]");
      }
